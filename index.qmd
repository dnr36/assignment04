...
institutions <- institutions %>%
    mutate(new_exec = lead(yrsoffc, default = NA),
           length_tenure = ifelse(new_exec == 1, 1, 0))
...
## A deeper look into the tenures in the least democratic country: Afghanistan
...
The graph below illustrates the frequency of tenures of different lengths. We can see that a lot of tenures are shorter than 5 years. Note how the bubbles for shorter tenures are bigger than for longer tenures such as anything longer than 10 years. This demonstrates the high levels of instability in the country. 


```{r, echo = FALSE}
summary_data <- institutions %>%
    filter(length_tenure == 1, yrsoffc != -999) %>%
    count(yrsoffc)
